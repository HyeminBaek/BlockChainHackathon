...
async function genSignedRawTx(type, to, amount) {
  try {
    ...
    } else if (type == 'ERC' || type == 'erc' || type =='RH-' || type=='rh-') {
...
(function excution(excute) {
  ...
  if (excute == 'transfer') {
    ...
    if(type == 'RH-' || type == 'rh-') tokenTransfer(to, amount)
  ...
  if (excute == 'generate') {
    ...
    if (type == 'ETH' || type == 'eth' || type == 'ERC' || type == 'erc' || type=='RH-'|| type=='rh-') genSignedRawTx(type, to, amount)
       }
}(process.argv[2]))
